...
class TestMatchingEngine:
    ...
    async def test_add_limit_buy_order_no_match(
        ...
        book_order = await self.matching_engine.get_order(
            ContractCode.UK_BL_MAR_25,
            limit_buy_order.id,
        )
        assert book_order.status == OpenOrderStatus.OPEN
        assert book_order.remaining_quantity == limit_buy_order.quantity
        assert book_order.id == limit_buy_order.id
    ...
    async def test_add_limit_sell_order_no_match(
        ...
        assert book_order.status == OpenOrderStatus.OPEN
        assert book_order.remaining_quantity == limit_sell_order.quantity
        assert book_order.id == limit_sell_order.id
    ...
    async def test_match_limit_orders_exact_quantity(
        ...
        if filled_sell_order:
            assert ProcessedOrderStatus.FILLED == filled_sell_order.status
            assert 0 == filled_sell_order.remaining_quantity

        if filled_buy_order:
            assert ProcessedOrderStatus.FILLED == filled_buy_order.status
            assert 0 == filled_buy_order.remaining_quantity
        ...
        assert 1 == len(trades)
        assert limit_buy_order.id == trades[0].buy_order_id
        assert limit_sell_order.id == trades[0].sell_order_id
        assert limit_buy_order.quantity == trades[0].quantity
        assert limit_sell_order.quantity == trades[0].quantity
        assert limit_buy_order.price == trades[0].price
        assert limit_sell_order.price == trades[0].price
